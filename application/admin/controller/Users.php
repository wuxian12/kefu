...
class Users extends Base
{
    ...
    {
        if(request()->isAjax()){
            $user_name = input('get.searchText','');
            if($user_name){
                $result = db('users')->where('user_name','like','%'.$user_name.'%')->order('id', 'desc')->select();
                $return['total'] = db('users')->where('user_name','like','%'.$user_name.'%')->count();  //总数据
            }else{
    ...
    public function addUser()
    {
        if(request()->isPost()){
            ...
            $param['user_pwd'] = md5($param['user_pwd'] . config('web.salt'));
            $param['online'] = 2; // 离线状态
