...
class Login extends Controller
{
    ...
    public function doLogin()
    {
        if(request()->isAjax()){
            ...
            if(md5($password . config('web.salt')) != $user['user_pwd']){
                return json(['code' => -2, 'data' => '', 'msg' => '密码错误']);
            }

            // 设置session标识状态
            cookie('l_user_name', $user['user_name'], config('web.save_time'));
            cookie('l_user_id', $user['id'], config('web.save_time'));
            cookie('l_user_avatar', $user['user_avatar'], config('web.save_time'));
