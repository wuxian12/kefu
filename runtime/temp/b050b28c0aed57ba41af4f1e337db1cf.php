<?php /*a:1:{s:53:"F:\www\kefu\application\service\view\index\index.html";i:1571123972;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
...
<script>
    var uid = "<?php echo htmlentities($uinfo['id']); ?>";
    var uname = "<?php echo htmlentities($uinfo['user_name']); ?>";
    var avatar = "<?php echo htmlentities($uinfo['user_avatar']); ?>";
    var group = "<?php echo htmlentities($uinfo['group_id']); ?>";
    var socket_server = "<?php echo htmlentities($socket); ?>";

    // 获取服务用户列表
    $.getJSON("<?php echo url('index/getUserList'); ?>", function(res){
        if(1 == res.code && res.data.length > 0){
            ...
            $.getJSON("<?php echo url('index/getCity'); ?>", {ip: ip}, function(res){
                $("#f-area").val(res.data);
            });
...
<script type="text/javascript" src="/static/service/js/laykefu.js"></script>
</body>
</html>